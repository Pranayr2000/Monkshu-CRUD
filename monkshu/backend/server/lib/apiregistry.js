...
function initSync() {
    ...
    for (const appObj of apps) {
        ...
        if (fs.existsSync(`${appRoot}/${CONSTANTS.API_MANAGER_DECODERS_CONF}`)) decoderPathAndRoots.push({ path: `${appRoot}/${CONSTANTS.API_MANAGER_DECODERS_CONF}`, root: appRoot });
        if (fs.existsSync(`${appRoot}/${CONSTANTS.API_MANAGER_ENCODERS_CONF}`)) encoderPathAndRoots.push({ path: `${appRoot}/${CONSTANTS.API_MANAGER_ENCODERS_CONF}`, root: appRoot });
        if (fs.existsSync(`${appRoot}/${CONSTANTS.API_MANAGER_HEADERMANAGERS_CONF}`)) headermanagersPathAndRoots.push({ path: `${appRoot}/${CONSTANTS.API_MANAGER_HEADERMANAGERS_CONF}`, root: appRoot });
        if (fs.existsSync(`${appRoot}/${CONSTANTS.API_MANAGER_SECURITYCHECKERS_CONF}`)) securitycheckersPathAndRoots.push({ path: `${appRoot}/${CONSTANTS.API_MANAGER_SECURITYCHECKERS_CONF}`, root: appRoot });
    }
    ...
    for (const decoderThis of decoders)
        if (decoderThis.initSync) decoderThis.initSync(apireg);
    for (const securitycheckerThis of securitycheckers)
        if (securitycheckerThis.initSync) securitycheckerThis.initSync(apireg);
    for (const headermanagerThis of headermanagers)
        if (headermanagerThis.initSync) headermanagerThis.initSync(apireg);
    for (const encoderThis of encoders)
        if (encoderThis.initSync) encoderThis.initSync(apireg);
...
function decodeIncomingData(url, data, headers, servObject) {
    const endPoint = urlMod.parse(url, true).pathname;
    const apireg = CLUSTER_MEMORY.get(API_REG_DISTM_KEY);
    let apiregentry = apireg[endPoint];
    if (!apiregentry) return false;
...
function _loadSortedConfOjbects(pathAndRoots) {
    ...
    for (const { path, root }
        ...
        for (const key of Object.keys(rawObject)) sortedConfObjects.push({ "module": `${root}/lib/apiregistry_extensions/${key.toLowerCase()}.js`, "priority": rawObject[key] });
    }
